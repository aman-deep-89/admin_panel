            ...
            <div class="row">
                <div class="col-md-12">
                  @if($errors->any())
                      ...
                      @endforeach
                  @endif
                <form action="{{route('product.update',$product->id) }}" class="needs-validation" method="post">
                  @method('PATCH')
                  @csrf
                <fieldset class="form-group">
                  <label for="product_name">Product Name *</label>
                  <input type="text" class="form-control" id="product_name" name="product_name" required  placeholder="Enter Product name" value="{{ old('product_name',$product->name) }}">
                </fieldset>

                <fieldset class="form-group">
                  <label for="product_desc">Description</label>
                  <small class="text-muted"><i>optional</i></small>
                  <input type="text" class="form-control" id="product_desc" name="product_description" value="{{ old('product_description',$product->description) }}" />
                </fieldset>
                <fieldset class="form-group">
                ...
                <fieldset class="form-group">
                  ...
                  <input type="radio" id="p_enable_yes" name="p_enable" value="1" @if ($product->p_enable==1)
                      checked
                  @endif /> Yes
                  <input type="radio" id="p_enable_no" name="p_enable" value="0" @if ($product->p_enable==0)
                    checked
                  @endif /> No
                </fieldset>
                <fieldset class="form-group">
                  <label for="out_of_stock">Out of Stock</label><br/>
                  <input type="radio" id="out_of_stock_yes" name="out_of_stock" value="1" @if ($product->out_of_stock==1)
                      checked
                  @endif /> Yes
                  <input type="radio" id="out_of_stock_no" name="out_of_stock" value="0" @if ($product->out_of_stock==0)
                    checked
                  @endif /> No
                </fieldset>
                <fieldset class="form-group">
                ...
                <input type="hidden" id="filenames" name="filenames"  value="{{ old('filenames',$product->photos)}}" />
                </fieldset>                
