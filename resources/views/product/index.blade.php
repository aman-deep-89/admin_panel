...
@section('content')
<div class="row">
   <div class="col-md-12"><a href="{{ route('product.create') }}" class="btn btn-primary">Add New Product</a></div>
</div>
                            ...
                            <table class="table zero-configuration">
                               ...
                               <tbody>
                                @foreach($product as $comp)  
                                    ...
                                    <td>{{$comp->sku}}</td>  
                                    <td>{{$comp->price}}</td>  
                                    <td>{{$comp->closing_stock}}</td>  
                                    <td><?php echo $comp->out_of_stock ? '<span class="badge badge-danger">Out of Stock' : '<span class="badge badge-success">In Stock' ?></td>  
                                    <td><a href="{{ url('product/view_accounts', $comp->id)}}" class="btn btn-sm btn-info">View</a>
                                        <a href="{{ route('product.edit', $comp->id)}}" class="btn btn-sm btn-primary">Edit</a> <button type="button" class="btn btn-danger btn-sm delete" data-id="{{ $comp->id }}" data-name="{{$comp->name}}" data-route="{{ route('product.destroy', $comp->id) }}" data-toggle="modal" data-target="#deleteDep">Delete</button></td>  
                                    </tr>
                                @endforeach
                               </tbody>
                                <tfoot>
