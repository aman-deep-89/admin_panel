            ...
            <div class="row">
                <div class="col-md-12">
                  @if($errors->any())
                      ...
                      @endforeach
                  @endif
                  <form action="{{route('user.update',$user->id)}}" class="needs-validation" method="post">
                  <div class="row">
                    <div class="col-md-8">    
                    @method('PATCH')
                    @csrf
                    <fieldset class="form-group">
                      ...
                      <input type="text" class="form-control username @error('email') is-invalid @enderror" id="username" name="username"  required placeholder="Enter username" value="{{ old('username',$user->username) }}">
                      <div id="username_error" class="text-danger"></div>
                    </fieldset>
                    <fieldset class="form-group">
                      <label for="first_name">User First Name *</label>
                      <input type="text" class="form-control @error('email') is-invalid @enderror" id="first_name" name="first_name"  required placeholder="Enter first_name" value="{{ old('first_name',$user->first_name) }}">
                    </fieldset>
                    <fieldset class="form-group">
                      <label for="last_name">User Last Name *</label>
                      <input type="text" class="form-control" id="last_name" name="last_name" required  placeholder="Enter last_name" value="{{ old('last_name',$user->last_name) }}">
                    </fieldset>
                    <fieldset class="form-group">
                      <label for="phone_no">Phone no *</label>
                      <input type="text" class="form-control" id="phone_no" name="phone_no" required  placeholder="Enter Phone No" value="{{ old('phone_no',$user->phone_number) }}">
                    </fieldset>
                    <fieldset class="form-group">
                      <label for="alternate_phone_no">Alternate Phone no <small>(optional)</small></label>
                      <input type="text" class="form-control" id="alternate_phone_no" name="alternate_phone_no"  placeholder="Enter Alternate Phone number" value="{{ old('alternate_phone_no',$user->alternate_phone_number) }}">
                    </fieldset>
                    <fieldset class="form-group">
                      <label for="internal_data">Internal Data *</label>
                      <input type="text" class="form-control" id="internal_data" name="internal_data" required placeholder="Enter Internal data" value="{{ old('internal_data',$user->internal_data) }}">
                    </fieldset>
                    <fieldset class="form-group">
                      <label for="vendor">Vendor *</label>
                      <input type="text" class="form-control" id="vendor" name="vendor" required placeholder="Enter Vendor" value="{{ old('vendor',$user->vendor) }}">
                    </fieldset>
                    <fieldset class="form-group">
                      <label for="business_type">Business Type *</label>
                      <input type="text" class="form-control" id="business_type" name="business_type" required placeholder="Enter business type" value="{{ old('business_type',$user->business_type) }}">
                    </fieldset>
                  ...
                  <div class="col-md-4">
                    ...
                    <div class="image-wrapper" style="height:220px;">
                      <img src="{{ $user->profile_img!='' ? asset('images/avatar/'.$user->profile_img) :asset('images/logo/logo.png') }}" class="avatar image1" width="200px" height="200px">
...
$(function() {
  $('#username').keyup(function(e) {
    ...
    $.ajax({
      ...
      data:"name="+txt+"&id={{$user->id}}&_token={{ csrf_token() }}",
      type:'post',
      dataType:'json',
      beforeSend:function() {
          $('#username_error').html("");
