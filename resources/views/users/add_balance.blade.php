          ...
          <div class="card-body">
            <div class="row">
                <div class="col-md-12"> 
                    ...
                    @endforeach
                @endif  
                <form action="{{url('user/add_balance') }}" class="needs-validation" method="post">
                <div class="row">
                  <div class="col-md-8">               
                        @csrf
                    <fieldset class="form-group">
                      <label for="first_name">User</label>
                      {!! Form::select('user_id', $users, 0, ['class' => 'form-control required chosen-select']) !!}
                    </fieldset>
                    <fieldset class="form-group">
                      <label for="balance_amount">Balance Amount</label>
                      <input type="number" min="1" step="0.01" class="form-control" id="balance_amount" name="balance_amount" required  placeholder="Enter Amount" value="{{ old('balance_amount') }}">
                    </fieldset>
                    <fieldset class="form-group">
                      <label for="date">Date</label>
                      <input type="text" class="form-control" id="date" name="date" required  placeholder="Enter Date" value="{{ old('date') }}">
                    </fieldset>
                    <fieldset class="form-group">
                      <label for="description">Description <small>optional</small></label>
                      <textarea class="form-control" id="description" name="description" required  placeholder="Enter description">{{ old('description') }}</textarea>
                    </fieldset>
