                ...
                <div class="card-content">
                    <div class="card-body card-dashboard">
                        ...
                        <div class="row">
                            ...
                            <div class="col-3"> {{$detail->users->username}} ({{$detail->users->email}}) </div>
                            <div class="col-3"> Product </div>
                            <div class="col-3"> {{$detail->products->name}} </div>
                            <div class="col-3"> Total Accounts </div>
                            <div class="col-3"> {{$detail->quantity}} </div>
                            <div class="col-3"> Price </div>
                            <div class="col-3"> {{$detail->total_price}} </div>
                        </div>
                            <div class="mt-1 pt-2 border-top">
                                ...
                                <?php $flag=false; ?>
                                    ...
                                    @if ($item->pd_status=='pending')  
                                    <?php $flag=true ?>
                                    <div class="row d-flex mt-1" id="row{{$key}}">
                                        <input type="hidden" required name="detail[{{$key}}][pd_id]" value="{{$item->pd_id}}" />
                                        <div class="col-md-2"><input type="text" required class="form-control" name="detail[{{$key}}][username]" placeholder="Username"/></div>
                                        <div class="col-md-2"><input type="text" required class="form-control" name="detail[{{$key}}][password]" placeholder="Password" /></div>  
                                        <div class="col-sm-12 col-md-3">
                                            <fieldset class="form-group position-relative has-icon-left">
                                                <input type="text" class="form-control spickadate" required data-id="{{$key}}" id="start_date{{$key}}" name="detail[{{$key}}][start_date]" placeholder="Select Start Date">
                                        ...
                                        <div class="col-sm-12 col-md-3">
                                            <fieldset class="form-group position-relative has-icon-left">
                                                <input type="text" class="form-control epickadate" required data-id="{{$key}}" id="end_date{{$key}}"  name="detail[{{$key}}][end_date]" placeholder="Select End Date">
                                                <div class="form-control-position">
                                                  <i class='bx bx-calendar'></i>
                                                </div>
                                              </fieldset>
                                        </div>
                                        <div class="col-md-2"><span id="remaining_days{{$key}}"></span>
                                            <input type="number" step="0.01" class="form-control" required id="cost{{$key}}"  name="detail[{{$key}}][cost]" placeholder="Enter Cost">
                                            <select class="form-control accept" data-id="{{$key}}" id="accept{{$key}}" name="detail[{{$key}}][status]">
        ...
        $('#save_account').submit(function(e) {
            ...
            $.ajax({
                ...
                success:function(res) {
                    if(res.success) {
                        window.location="{{ url('user/purchase_requests') }}"
                    } else {
                        var str='';
