          ...
          <div class="card-body">
            <div class="row">
                <div class="col-md-12">
                  ...
                  <?php endif; ?>
                <form action="<?php echo e(route('product.update',$product->id)); ?>" class="needs-validation" method="post">
                  <?php echo method_field('PATCH'); ?>
                  <?php echo csrf_field(); ?>
                <fieldset class="form-group">
                  <label for="product_name">Product Name *</label>
                  <input type="text" class="form-control" id="product_name" name="product_name" required  placeholder="Enter Product name" value="<?php echo e(old('product_name',$product->name)); ?>">
                </fieldset>

                <fieldset class="form-group">
                  <label for="product_desc">Description</label>
                  <small class="text-muted"><i>optional</i></small>
                  <input type="text" class="form-control" id="product_desc" name="product_description" value="<?php echo e(old('product_description',$product->description)); ?>" />
                </fieldset>
                <fieldset class="form-group">
                  <label for="sku">SKU</label>
                  <input type="text" class="form-control" id="sku" name="sku" required value="<?php echo e(old('sku',$product->sku)); ?>" />
                </fieldset>
                <fieldset class="form-group">
                  <label for="price">Price</label>
                  <input type="number" step="0.01" class="form-control" id="price" name="price" required value="<?php echo e(old('price',$product->price)); ?>" />
                </fieldset>
                <fieldset class="form-group">
                  <label for="p_enable">Active</label><br/>
                  <input type="radio" id="p_enable_yes" name="p_enable" value="1" <?php if($product->p_enable==1): ?>
                      checked
                  <?php endif; ?> /> Yes
                  <input type="radio" id="p_enable_no" name="p_enable" value="0" <?php if($product->p_enable==0): ?>
                    checked
                  <?php endif; ?> /> No
                </fieldset>
                <fieldset class="form-group">
                  <label for="out_of_stock">Out of Stock</label><br/>
                  <input type="radio" id="out_of_stock_yes" name="out_of_stock" value="1" <?php if($product->out_of_stock==1): ?>
                      checked
                  <?php endif; ?> /> Yes
                  <input type="radio" id="out_of_stock_no" name="out_of_stock" value="0" <?php if($product->out_of_stock==0): ?>
                    checked
                  <?php endif; ?> /> No
                </fieldset>
                <fieldset class="form-group">
                ...
                <input type="hidden" id="filenames" name="filenames"  value="<?php echo e(old('filenames',$product->photos)); ?>" />
                </fieldset>                
    ...
    <script type="text/javascript">
        $(function() {
          ...
          $ele.fileinput({
              ...
              initialPreview:[ 
                <?php  
                $files=json_decode(old('filenmes',$product->photos),true);
              ...
              initialPreviewConfig: [
                  ...
                  foreach($files as $f) { echo '{"type":"';  $ext = pathinfo($f, PATHINFO_EXTENSION);
                      if(in_array($ext,array('jpg','gif','png','jpeg','jpe'))) echo 'image",'; else echo 'pdf",'; echo '"caption":"'.$f.'","url":"'.url('delete_file').'","key":"'.$f.'"},';
                  } };    
                  ?>
            ],
              /*initialPreviewConfig: [
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.contentLayoutMaster', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\admin_mgmt\resources\views/product/edit.blade.php ENDPATH**/ ?>